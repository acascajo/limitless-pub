...
/**
	Get the temperature features for each coretemp, calculate percentage on the maximum temperature and concat
	the current temperature and the percentage on the maximum temperature
...
/**
 * Same than before but only works in raspberry pi 5 with ubuntu
 */
int get_temperature_pi();

/**
 * Same than before but only works with AMD HPC with rocky linux
 */
