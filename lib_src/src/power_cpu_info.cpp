...
std::string exec_command(const std::string &comando) {
    ...
    while (fgets(buffer, sizeof(buffer), pipe) != nullptr) {
        resultado = buffer; // Guarda solo la última línea
    }
    pclose(pipe);
    return resultado;
}
...
int get_power_pi(){
	std::string comando = "python3 ~/power_meter.py | tail -n 1 | awk '{print $(NF-1)}'"; 
    	std::string powerwatts = exec_command(comando);

       	float watts = stof(powerwatts);
	log_concat(watts, 4);
